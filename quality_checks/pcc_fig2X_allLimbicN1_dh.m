...
for ss = 1:nr_subs % subject loop
    ...
    all_out(ss).tR = NaN(size(all_out(ss).crp_out));
    ...
    for kk = 1:length(these_measured_sites)
        ...
        for ll = 1:length(these_stim_sites)
            if ~isempty(all_out(ss).crp_out(these_measured_sites(kk), these_stim_sites(ll)).data) % ~ same stim/recording site
                ...
                all_out(ss).tR(these_measured_sites(kk), these_stim_sites(ll)) = all_out(ss).crp_out(these_measured_sites(kk), these_stim_sites(ll)).crp_parms.tR;
            else
            ...
            for kk = 1:length(these_measured_sites)
                ...
                for ll = 1:length(these_stim_sites)
                    if ~isempty(all_out(ss).crp_out(these_measured_sites(kk), these_stim_sites(ll)).data) % ~ same stim/recording site
                        ...
                        % store subject index
                        out(measure_ind,stim_ind).subj_ind(resp_counter,1) = ss;
                        ...
                        out(measure_ind,stim_ind).tR(resp_counter, :) = all_out(ss).tR(these_measured_sites(kk), these_stim_sites(ll)); 
                    end
...
% area_names = {'Hipp','Amyg','PCC','ACC'};   
area_color = {[1 .5 0],[1 0 1],[0 .5 1],[1 1 0]};
...
for measure_ind = 1:length(area_codes)
    for stim_ind = 1:length(area_codes)
        ...
        bar(length(out(measure_ind,stim_ind).p),'FaceColor',[.99 .99 .99]);
        bar(sum(sign_resp),'FaceColor',min(area_color{measure_ind}+[.2 .2 .2],[1 1 1]));
        bar(sum(n1s),'FaceColor',area_color{measure_ind}./2);
        ylim([0 1.1*length(out(measure_ind,stim_ind).p)])
    end
end
