...
set_color = {[0 0.4470 0.7410],...  % blue
...
% Set connection to plot woth stim and meas ROI:
...
for ss = 1:8 
    ...
    % find waves for this particular connection  
    ...
    if isempty(this_set)
        ...
        plot(tt, ss + this_set, 'Color',sub_color{ss}, 'LineWidth', .5); % plot waveforms 
        xlim([-0.2 .6]), ylim([sub_axis{ss}]);
        ylabel('Voltage (uV)')

        ss_sign = width(this_set);                          % number of of sig responses
        txt = 100*ss_sign/sum(ss_resps)                     % get proportion of sign responses
