...
out = []; % this will be a area X area structure, with all subjects concatinated for each area
...
compare(lme3,lme2,'CheckNesting',true,'NSim',1000);       % Check nested conditions between models with 1000 replications     
compare(lme,lme3,'CheckNesting',true,'NSim',1000);          % Check nested conditions between models with 1000 replications     
...
anova(lme);                           % perform F-test that all fixed-effects coefficients are zero
anova(lme,'DFMethod','satterthwaite') % degrees of freedom w Satterthwaite method
...
plotSpread(y, 'distributionIdx', plot_groups, 'xValues',1:4,...
    'spreadWidth',.5, 'distributionColors',[0.5, 0.5, 0.5]);
