...
% load events - already loaded before
% events_table = readtable(events_tsv_name,'FileType','text','Delimiter','\t','TreatAsEmpty',{'N/A','n/a'});
...
crp_out = [];
% cross_ccep_t = NaN(nr_channels,max(stim_pair_nr)); % cod for each response
% cross_ccep_p = NaN(nr_channels,max(stim_pair_nr)); % signrank on cod for each response
...
for kk = 1:max(stim_pair_nr) % condition number
    ...
    if ~isempty(these_epochs)
        ...
        if use_CAR == 1
%             [signaldata] = ccepPCC_CAR64blocks(signaldata,tt,good_channels);
            perc_channels = 0.2;
